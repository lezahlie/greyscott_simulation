...
def add_simulation_group(parser):
    ...
    group.add_argument('--patch-radius', dest='patch_radius', type=int, default=20,
                            help="Half-width of central perturbation | default: 20")
    
    group.add_argument('--patch-prob', dest='patch_prob', type=float, default=0.5,
        help="Probability of placing each patch (default: 0.5)")
    
    # group.add_argument('--tolerance', dest='tolerance', type=float, default=1e-4,
    #                         help="Tolerance value for determining simulation convergence. | default: 1e-4")
...
def check_simulation_args(args):
    ...
    if not (0 <= args.patch_prob <= 1.0):
        raise ap.ArgumentError(None, f"PATCH_PROB must be a float inclusively between [0, 1]")

    grid_radius = args.grid_length // 2
    if not (1 <= args.patch_radius <= grid_radius):
        raise ap.ArgumentError(None, f"PATCH_RADIUS must be an INT inclusively between [1, {grid_radius}]")
...
def process_args(exe_file):
    ...
    parser = ap.ArgumentParser(description="Grey Scott Reaction-Diffusion Simulation", formatter_class=ap.RawTextHelpFormatter)
